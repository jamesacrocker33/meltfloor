...
{"title": "max 80 chars, keyword-front-loaded",
 ...
 "keywords": ["search terms buyers actually type"]}` +
  ...
  if (data.error) return json({ error: "AI error: " + data.error.message }, 502);
...
function render(L){
  ...
  $("out").innerHTML=
    card("Title ("+L.title.length+"/80)",L.title)+
    card("Description",L.description)+
    card("Condition",L.condition)+
    (specs?card("Item specifics",specs):"")+
    '<div class="card"><h3>Pricing<button class="copy" onclick="cp(this)">COPY</button></h3>'+
    '<div class="val"><span class="price">$'+(+L.price.suggested_list).toFixed(2)+'</span>'+
    '<div class="floor">Floor $'+(+L.price.floor).toFixed(2)+' — '+esc(L.price.reasoning)+'</div></div></div>'+
    (L.keywords?card("Keywords",(L.keywords||[]).join(", ")):"");}
